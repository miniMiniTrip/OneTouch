...
document.addEventListener('DOMContentLoaded', function() {
    // ============ 서버에서 전달받은 값 ============
    const serverTab = '${defaultTab}';
    const serverPostIdx = '${targetPostIdx}';
    
    // ============ URL 파라미터 우선순위: URL > 서버 전달값 ============
    const urlParams = new URLSearchParams(window.location.search);
    const urlTab = urlParams.get('tab');
    const urlPostIdx = urlParams.get('post_idx');
    
    // 최종값 결정
    const tab = urlTab || (serverTab !== '' ? serverTab : null);
    const targetPostIdx = urlPostIdx || (serverPostIdx !== '' ? serverPostIdx : null);
    
    console.log('=== 포스트 페이지 로드 ===');
    console.log('tab:', tab, 'post_idx:', targetPostIdx);
    
    // 탭클릭
    if (tab) {
        const tabElement = document.getElementById('post-' + tab);
        if (tabElement) {
            console.log('탭 자동 클릭:', tab);
            tabElement.click();
            
            // 포스트 스크롤 AJAX 로드 대기
            if (targetPostIdx) {
                setTimeout(() => {
                    scrollToPost(targetPostIdx);
                }, 1500);
            }
        }
    } else {
...
function scrollToPost(postIdx) {
    console.log('포스트 스크롤 시도:', postIdx);
    
    const postElement = document.querySelector(`[data-post-idx="${postIdx}"]`);
    
    if (postElement) {
        console.log('포스트 발견:', postIdx);
        ...
        console.log('포스트를 찾을 수 없음. 재시도 중...');
        
        // 재시도 (AJAX 로딩이 느릴 경우 대비)
        setTimeout(() => {
            const retryElement = document.querySelector(`[data-post-idx="${postIdx}"]`);
            if (retryElement) {
                console.log('재시도 성공:', postIdx);
                retryElement.scrollIntoView({ 
                    behavior: 'smooth', 
                    block: 'center' 
                });
                retryElement.style.transition = 'background-color 0.5s ease';
                retryElement.style.backgroundColor = '#fff9e6';
                setTimeout(() => {
                    retryElement.style.backgroundColor = '';
                }, 2000);
            } else {
                console.log('재시도 실패. 포스트가 존재하지 않을 수 있습니다.');
            }
        }, 2000);
    }
} 
...
function listHtml(post_category, nowPage) {
    console.log(`listHtml 호출: category=${post_category}, page=${nowPage}`);
    
    $.ajax({
        url: "/post/all_list",
        data: {
            "tabType": post_category,
            "page": nowPage
        },
        success: function(d) {
            let html = "";
            ...
            // 포스트 HTML 생성
            postVo_array.forEach(function(postData) {
                // ⭐ 여기에 data-post-idx 속성 추가 ⭐
                html += `
                <div class="community-post" data-post-idx="${postData.post_idx}">
                    <div class="post-header">
                        <img src="${postData.mem_image_url ? contextPath + '/images/members/' + postData.mem_image_url : contextPath + '/images/profile.png'}" 
                             alt="프로필" class="profile-img">
                        <span class="username">${postData.mem_id || '익명'}</span>
                `;
                
                // 본인 게시물일 때만 수정/삭제 버튼
                if (postData.mem_idx === login_mem_idx) {
                    html += `
                        <div class="more-options-wrapper">
                            <button class="more-options">⋮</button>
                            <div class="action-menu">
                                <button class="edit-btn" data-post-idx="${postData.post_idx}">수정</button>
                                <button class="delete-btn" data-post-idx="${postData.post_idx}">삭제</button>
                            </div>
                        </div>
                    `;
                }
                
                html += `</div>`; // post-header 종료
                
                // 이미지 캐러셀 (이미지가 있을 때만)
                if (postData.post_image) {
                    const images = postData.post_image.split('*');
                    html += `
                    <div class="image-carousel" data-post-idx="${postData.post_idx}">
                    ...
                    images.forEach((img, index) => {
                        html += `
                            <div class="carousel-item ${index === 0 ? 'active' : ''}">
                                <img src="${contextPath}/images/posts/${img}" 
                                     alt="게시물 이미지" 
                                     class="post-image">
                            </div>
                        `;
                    ...
                    html += `</div>`; // carousel-inner 종료
                    
                    // 이미지가 2개 이상일 때만 컨트롤 버튼 표시
                    if (images.length > 1) {
                        html += `
                            <button class="carousel-control prev" onclick="moveSlide(${postData.post_idx}, -1)">
                                <i class="lni lni-chevron-left"></i>
                            </button>
                            <button class="carousel-control next" onclick="moveSlide(${postData.post_idx}, 1)">
                        ...
                        images.forEach((img, index) => {
                            html += `
                                <span class="indicator ${index === 0 ? 'active' : ''}" 
                                      onclick="goToSlide(${postData.post_idx}, ${index})"></span>
                            `;
                        });
                        
                        html += `</div>`; // carousel-indicators 종료
                    }
                    
                    html += `</div>`; // image-carousel 종료
                }
                
                // ... 나머지 포스트 내용 (좋아요, 댓글 등) ...
                
                html += `</div>`; // community-post 종료
            });
            
            // HTML 삽입
            document.querySelector('.community-content').innerHTML = html;
