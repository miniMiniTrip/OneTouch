...
public class UserQnaController {
    ...
    public String insertQna(QnaVo vo, int product_idx) {
        // 로그인 체크
        MemVo user = (MemVo) session.getAttribute("user");
    ...
    public String tt(Model model) {
        ...
        System.out.println("	[QnaController] return : qna/qna_list.jsp ");
        System.out.println("");
        return "/product/product_qna_list";
    }

    // ==================== 상품 상세페이지 Q&A (새로 추가) ====================
    
    // 상품별 Q&A 목록 조회
    @GetMapping("/product/qna_list")
    public String productQnaList(@RequestParam("product_idx") int product_idx, Model model) {
        System.out.println("   [UserQnaController] productQnaList() - product_idx: " + product_idx);
        ...
        System.out.println("   상품 Q&A 목록 개수: " + qna_list.size());
        
        model.addAttribute("qna_list", qna_list);
        model.addAttribute("product_idx", product_idx);
        
        System.out.println("   [UserQnaController] return : product/product_qna_list.jsp");
        System.out.println("");
        return "product/product_qna_list";
    }
    
    // 상품 Q&A 작성 폼
    @GetMapping("/product/qna_write")
    public String productQnaWrite(@RequestParam("product_idx") int product_idx, Model model) {
        System.out.println("   [UserQnaController] productQnaWrite() - product_idx: " + product_idx);
        ...
        model.addAttribute("product_idx", product_idx);
        model.addAttribute("isProductQna", true); // 상품 Q&A 구분용
        
        System.out.println("   [UserQnaController] return : qna/qna_write.jsp (상품용)");
        System.out.println("");
        return "qna/qna_write"; // 기존 폼 재사용
    }
    
    // 상품 Q&A 등록 처리
    @PostMapping("/product/qna_write")
    public String insertProductQna(QnaVo vo, @RequestParam("product_idx") int product_idx) {
        System.out.println("   [UserQnaController] insertProductQna() - product_idx: " + product_idx);
        ...
        // 작성자 정보 설정
        vo.setMem_idx(user.getMem_idx());
        vo.setMem_idx(product_idx); // 상품 번호 설정
        
        System.out.printf("   상품 qna추가 vo:%s\n", vo);
        qnaDao.insertQna(vo); // 기존 메서드 재사용
        
        System.out.println("   [UserQnaController] redirect:/product/qna_list?product_idx=" + product_idx);
        System.out.println("");
        // 등록 후 해당 상품 Q&A 목록으로 리다이렉트
        return "redirect:/product/qna_list?product_idx=" + product_idx;
    }
}